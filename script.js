...
function age(){let v=document.getElementById('dob').value;if(!v){o('ar','Select date.');return}let d=new Date(v),x=new Date();if(d>x){o('ar','Invalid date.');return}let y=x.getFullYear()-d.getFullYear(),m=x.getMonth()-d.getMonth();if(x.getDate()<d.getDate())m--;if(m<0){y--;m+=12}o('ar',`Age: ${y} years ${m} months`)}
...
function si(){let p=n('sp'),r=n('sr'),t=n('st'),i=p*r*t/100;o('sir',isFinite(i)?`Interest: ₹${i.toFixed(2)} | Total: ₹${(p+i).toFixed(2)}`:'Enter valid values.')}
function ci(){let p=n('cp'),r=n('cr'),t=n('ct'),a=p*(1+r/100)**t;o('cir',isFinite(a)?`Interest: ₹${(a-p).toFixed(2)} | Amount: ₹${a.toFixed(2)}`:'Enter valid values.')}
function salary(){let m=n('sal');o('salr',m>=0?`Annual salary: ₹${(m*12).toFixed(2)}`:'Enter valid salary.')}
function loan(){let p=n('lp'),r=n('lr'),t=n('lt'),i=p*r*t/100;o('lrx',isFinite(i)?`Interest: ₹${i.toFixed(2)} | Total: ₹${(p+i).toFixed(2)}`:'Enter valid values.')}
